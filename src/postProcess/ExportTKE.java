...
public class ExportTKE extends StarMacro {

    static final String FIELDFUNCTION = "TurbulentKineticEnergy";
    static final String OUTPUTNAME = "TKE";
    ...
    public void execute() {
        ...
        createTKEScene("TKE", false, 1);
    }

    private void createTKEScene(String name, Boolean recordScene, int frequency) {
        ...
        ScalarDisplayer displayer = ((ScalarDisplayer) scene.getDisplayerManager().getDisplayer(name));
        displayer.initialize();
        addFieldFunctionToScalarDisplayer(displayer, FIELDFUNCTION);
        addDerivedPartsToDisplayer(displayer, "XZ Plane");

        String RPM = getRpmString();
        String RotorName = getRotorDiameterString();
        String targetPath = String.format("%s/Figs/%s.png", SessionDirectory, OUTPUTNAME);
    ...
    private void addFieldFunctionToVectorDisplayer(VectorDisplayer vectorDisplayer, String fieldFunctionName) {
        ...
        vectorDisplayer.getVectorDisplayQuantity().setMinMagnitude​(0.0);
    }
